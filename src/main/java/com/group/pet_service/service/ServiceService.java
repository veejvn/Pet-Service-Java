...
public class ServiceService {
    ...
    public ServiceResponse updateService(String id, ServiceRequest serviceRequest) {
        // Tìm dịch vụ theo ID
        Service existingService = petServiceRepository.findById(id)
                .orElseThrow(() -> new AppException(HttpStatus.NOT_FOUND, "Service not found", "service-s-01"));
        ...
        Service updatedService = petServiceRepository.save(existingService);
    ...
    public List<ServiceResponse> getLatestServices(int count) {
        List<Service> latestServices = petServiceRepository.findAll(Sort.by(Sort.Direction.DESC, "createAt"))
                .stream()
                .limit(count)
                .toList();
